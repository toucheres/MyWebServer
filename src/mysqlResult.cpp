...
void MySQLResult::bindResultBuffers()
{
    ...
    for (unsigned int i = 0; i < fieldCount; i++)
    {
        ...
        // 对于大型数据类型，增加缓冲区大小
        if (fields[i].type == MYSQL_TYPE_BLOB || fields[i].type == MYSQL_TYPE_VAR_STRING ||
...
std::vector<std::string> MySQLResult::getRow()
{
...
std::vector<std::vector<std::string>> MySQLResult::getRows()
{
    std::vector<std::vector<std::string>> rows;
    rows.reserve(getRowCount());
