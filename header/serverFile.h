...
{
  private:
    int protocolType_ = 0; // Default to 0 (HTTP)
  ...
  public:
    ...
    static std::unordered_map<int, ProtocolHandler>& getProtocolHandlers(); // Key type changed to int
    
    // 注册协议处理函数
    static bool registerProtocolHandler(int protocolType, ProtocolHandler handler); // Parameter type changed to int
  ...
  public:
    ...
    // 实现所有虚函数，不再是抽象类
    int getAgreementType() const; // Return type changed to int
    ...
    int handle();
    void closeIt();
    bool upgradeProtocol(int newProtocol); // Parameter type changed to int
